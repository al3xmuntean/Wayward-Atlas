    ...
    >
      ...
      >
        ...
        <button
          ...
          className="absolute top-5 right-5 p-2 text-slate-400 hover:text-white rounded-xl hover:bg-slate-800 transition-colors focus:ring-2 focus:ring-olive-500"
        >
        ...
        <div className="flex items-center gap-2.5 mb-1 text-olive-400">
          <div className="p-2 rounded-xl bg-olive-950/80 border border-olive-500/30 text-olive-400 shadow-sm">
            <Sparkles className="w-5 h-5" aria-hidden="true" />
          </div>
          <div>
            <h2 id="planner-modal-title" className="text-xl font-black text-white tracking-tight flex items-center gap-2">
        ...
        {error && (
          <div role="alert" className="mt-4 p-3.5 rounded-2xl bg-rose-950/60 border border-rose-800/60 text-xs text-rose-300">
            {error}
          </div>
        )}
        ...
        {!generatedPlan ? (
          <form onSubmit={handleGeneratePlan} className="space-y-4 mt-5">
            {/* Destination Input */}
            <div>
              <label htmlFor="planner-dest" className="text-xs font-bold text-slate-200 mb-1.5 flex items-center gap-1.5">
                <MapPin className="w-3.5 h-3.5 text-olive-400" aria-hidden="true" />
              ...
              <input
                ...
                className="w-full px-4 py-3 rounded-2xl bg-slate-900/90 border border-slate-700/80 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-olive-500 transition-all shadow-inner"
              />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label htmlFor="planner-duration" className="text-xs font-bold text-slate-200 mb-1.5 flex items-center justify-between">
                  <span className="flex items-center gap-1.5">
                    <Clock className="w-3.5 h-3.5 text-olive-400" aria-hidden="true" />
                    Durată călătorie:
                  </span>
                  <span className="text-olive-300 font-bold px-2 py-0.5 rounded-lg bg-olive-950/60 border border-olive-500/30">
                    {days} {days === 1 ? "zi" : "zile"}
                  </span>
                </label>
                <input
                  ...
                  className="w-full accent-olive-500 cursor-pointer"
                />
              </div>

              <div>
                <label htmlFor="planner-dates" className="text-xs font-bold text-slate-200 mb-1.5 flex items-center gap-1.5">
                  <Calendar className="w-3.5 h-3.5 text-olive-400" aria-hidden="true" />
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 rounded-xl bg-slate-900 border border-slate-800 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-olive-500 transition-colors"
                />
              </div>
            </div>
            ...
            <div>
              <span className="text-xs font-bold text-slate-200 mb-1.5 block" id="planner-style-label">
                Stilul călătoriei:
              </span>
              <div
                role="radiogroup"
                ...
                {TRAVEL_STYLES.map((s) => (
                  <button
                    ...
                    className={`flex items-center gap-2 p-2.5 rounded-xl text-xs font-semibold border transition-all text-left focus:ring-2 focus:ring-olive-500 ${
                      style === s.id
                        ? "bg-olive-700/30 border-olive-500 text-olive-200 shadow-sm"
            ...
            <div className="p-3.5 rounded-2xl bg-rose-950/20 border border-rose-900/40 flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <div className="p-2 rounded-xl bg-rose-900/50 text-rose-300">
                  <Heart className="w-4 h-4 text-rose-400 fill-rose-400/40" aria-hidden="true" />
                </div>
                <div>
            ...
            <div>
              <label htmlFor="planner-prompt" className="text-xs font-bold text-slate-200 mb-1 block">
              ...
              <textarea
                ...
                className="w-full px-3.5 py-2.5 rounded-xl bg-slate-900 border border-slate-800 text-xs text-white placeholder-slate-600 focus:outline-none focus:border-olive-500 transition-colors resize-none"
              />
            </div>
            ...
            <div>
              <button
                ...
                className="text-[11px] font-semibold text-slate-400 hover:text-olive-300 flex items-center gap-1 transition-colors focus:ring-2 focus:ring-olive-500 rounded p-1"
              >
              ...
              {showApiKeyInput && (
                <div className="mt-2 p-3 rounded-xl bg-slate-900/90 border border-slate-800 space-y-1 animate-fade-in">
                  <label htmlFor="gemini-api-key" className="text-[11px] text-slate-400 block">Cheie API Gemini:</label>
                  <input
                    id="gemini-api-key"
                    type="password"
                    value={apiKey}
                    ...
                    className="w-full px-3 py-2 rounded-lg bg-slate-950 border border-slate-700 text-xs text-olive-300 placeholder-slate-600 focus:outline-none focus:border-olive-400 font-mono"
                  />
                  <p className="text-[10px] text-slate-500">
                    Dacă serverul are deja <code className="text-slate-400">GEMINI_API_KEY</code> în variabilele de mediu, lasă liber.
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="space-y-4 mt-5 animate-fade-in max-h-[70vh] overflow-y-auto pr-1">
            {/* Plan Header Card */}
            <div className="p-4 rounded-2xl bg-gradient-to-br from-olive-950/50 to-slate-900 border border-olive-600/40 shadow-xl space-y-2">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <span className="p-2 rounded-xl bg-olive-900/60 text-olive-300">
                    <Compass className="w-5 h-5" aria-hidden="true" />
                  </span>
                  <div>
                    <h3 className="text-base font-black text-white">{generatedPlan.destination}</h3>
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[11px] font-bold px-2.5 py-1 rounded-full bg-olive-900/70 text-olive-200 border border-olive-500/40">
                    {generatedPlan.allTargetCheckpoints.length} Obiective Țintă
                  </span>
                </div>
              </div>

              <p className="text-xs text-slate-200 italic leading-relaxed pt-1 pl-2 border-l-2 border-olive-500/50">
            ...
            {generatedPlan.partnerTips && withPartner && (
              <div className="p-3.5 rounded-2xl bg-rose-950/30 border border-rose-800/40 space-y-1 animate-fade-in">
            ...
            {/* Tab Navigation */}
            <div className="flex items-center gap-1.5 p-1 rounded-2xl bg-slate-900 border border-slate-800" role="tablist" aria-label="Opțiuni vizualizare plan">
              <button
                type="button"
                role="tab"
              ...
              <div className="space-y-4 animate-fade-in">
                ...
                <div className="p-4 rounded-2xl bg-slate-900/80 border border-slate-800 space-y-2.5">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-olive-300 uppercase tracking-wider flex items-center gap-1.5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {generatedPlan.allTargetCheckpoints.map((cp, idx) => (
                      <div
                        key={idx}
                        className="flex items-start gap-2 p-2 rounded-xl bg-slate-950/80 border border-slate-800 text-xs text-slate-200"
                      >
                        <CheckCircle2 className="w-3.5 h-3.5 text-olive-400 shrink-0 mt-0.5" aria-hidden="true" />
                ...
                <div className="space-y-2.5">
                  <span className="text-xs font-bold text-slate-300 uppercase tracking-wider flex items-center gap-1.5">
                    <Layers className="w-3.5 h-3.5 text-olive-400" aria-hidden="true" />
                  ...
                  {generatedPlan.itinerary.map((day) => (
                    <div
                      key={day.dayNumber}
                      className="p-3.5 rounded-2xl bg-slate-900/70 border border-slate-800 hover:border-olive-500/30 transition-all space-y-2"
                    >
                      <div className="flex items-center justify-between">
                        <h4 className="text-xs font-bold text-white flex items-center gap-2">
                          <span className="w-5 h-5 rounded-full bg-olive-900/80 text-olive-300 flex items-center justify-center text-[10px] font-black border border-olive-500/40">
                            {day.dayNumber}
                          </span>
                          {day.title}
                        </h4>
                      ...
                      {day.highlights && day.highlights.length > 0 && (
                        <div className="flex flex-wrap gap-1.5 pl-7 pt-1">
                          {day.highlights.map((hl, hidx) => (
                            <span
                              key={hidx}
                              className="text-[10px] font-semibold px-2 py-0.5 rounded-md bg-slate-800 text-olive-300 border border-olive-500/20"
                            >
                              ✦ {hl}
                            </span>
                          ))}
            ...
            {activeTab === "packing" && (
              <div className="space-y-4 animate-fade-in">
                ...
                <div className="p-4 rounded-2xl bg-gradient-to-br from-olive-950/40 to-slate-900 border border-olive-600/30 space-y-2">
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-bold text-white flex items-center gap-2">
                      <Luggage className="w-4 h-4 text-olive-400" aria-hidden="true" />
                      Progres Împachetare Bagaj:
                    </span>
                    <span className="font-mono font-bold text-olive-300">
                    ...
                    filteredPackingItems.map((item) => (
                      <button
                        ...
                        className={`w-full flex items-center justify-between p-2.5 rounded-xl border text-xs transition-all text-left group ${
                          item.checked
                            ? "bg-olive-950/40 border-olive-700/50 text-slate-300 line-through opacity-80"
                            : "bg-slate-900/80 border-slate-800 hover:border-slate-700 text-white"
                        }`}
                      >
                        <div className="flex items-center gap-2.5">
                          {item.checked ? (
                            <CheckSquare className="w-4 h-4 text-olive-400 shrink-0" aria-hidden="true" />
                          ) : (
                            <Square className="w-4 h-4 text-slate-500 group-hover:text-olive-400 shrink-0" aria-hidden="true" />
                          )}
                          <span>{item.item}</span>
                        </div>
                        <span className="text-[10px] uppercase font-bold px-2 py-0.5 rounded-md bg-slate-800 text-slate-400">
                ...
                <form onSubmit={addCustomPackingItem} className="flex gap-2 pt-1">
                  <input
                    ...
                    className="flex-1 px-3 py-2 rounded-xl bg-slate-900 border border-slate-800 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-olive-500"
                  />
                  <button
                    type="submit"
                    disabled={!customItemText.trim()}
                    className="px-3.5 py-2 rounded-xl bg-olive-700 hover:bg-olive-600 text-white font-bold text-xs flex items-center gap-1 disabled:opacity-50 transition-colors"
                  >
            ...
            {activeTab === "weather" && (
              <div className="space-y-4 animate-fade-in">
                ...
                {generatedPlan.weatherForecastSummary && (
                  <div className="p-4 rounded-2xl bg-gradient-to-br from-amber-950/30 to-slate-900 border border-amber-800/40 space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold text-amber-300 flex items-center gap-1.5">
                        <CloudSun className="w-4 h-4 text-amber-400" aria-hidden="true" />
                        Climat & Temperaturi Estimate:
                      </span>
                      <span className="text-xs font-bold px-2.5 py-0.5 rounded-full bg-amber-900/60 text-amber-200 border border-amber-700/50">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                      {generatedPlan.localTips.map((tip, idx) => (
                        <div key={idx} className="p-3 rounded-xl bg-slate-900/80 border border-slate-800 space-y-1">
                          <h5 className="text-xs font-bold text-olive-300 flex items-center gap-1.5">
            ...
            <div className="pt-3 flex items-center gap-3">
              <button
                ...
                className="py-3 px-4 rounded-xl bg-slate-800 hover:bg-slate-700 text-xs font-bold text-slate-300 transition-colors focus:ring-2 focus:ring-slate-400"
              >
                Modifică Criteriile
              </button>
