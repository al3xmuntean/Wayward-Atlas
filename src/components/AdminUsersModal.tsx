    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3 mb-2">
          <div className="p-2.5 rounded-2xl bg-olive-500/20 text-olive-700 dark:text-olive-400 border border-olive-500/30">
            <Users className="w-6 h-6" aria-hidden="true" />
          </div>
          <div>
            <h2 id="admin-users-title" className="text-xl font-black text-slate-900 dark:text-white tracking-tight">
              Gestiune Roluri & Permisiuni Utilizatori
            </h2>
            <span className="text-xs text-olive-700 dark:text-olive-400 font-semibold">
        ...
        <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-1" role="list" aria-label="Lista utilizatorilor înregistrați">
          {loading ? (
            <div className="py-12 flex flex-col items-center justify-center text-olive-600 dark:text-olive-400 gap-2">
              <Loader2 className="w-6 h-6 animate-spin" aria-hidden="true" />
              <span className="text-xs">Se încarcă lista de utilizatori...</span>
            </div>
          ) : users.length === 0 ? (
            <div className="p-8 text-center bg-slate-100 dark:bg-slate-900/50 rounded-2xl border border-slate-200 dark:border-slate-800">
              <Users className="w-8 h-8 text-slate-400 dark:text-slate-600 mx-auto mb-2" aria-hidden="true" />
              <p className="text-sm font-semibold text-slate-800 dark:text-slate-300">Nu există alți utilizatori înregistrați încă.</p>
            ...
            users.map((u) => {
              ...
              return (
                <div
                  key={u.id}
                  role="listitem"
                  className="p-3.5 sm:p-4 rounded-2xl bg-white dark:bg-slate-900/70 border border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 transition-all hover:border-olive-500/40 dark:hover:border-slate-700 shadow-xs"
                >
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-sm text-slate-900 dark:text-white">{u.name}</span>
                      <span className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-md font-semibold bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-300 border border-slate-200 dark:border-slate-700">
                        {currentCfg.icon}
                        <span>{currentCfg.label}</span>
                      </span>
                    </div>
                    <span className="text-xs text-slate-600 dark:text-slate-400 block mt-0.5">{u.email}</span>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-2 w-full sm:w-auto">
                    ...
                    <div
                      ...
                      className="grid grid-cols-2 sm:flex sm:items-center gap-1 bg-slate-100 dark:bg-slate-950 p-1 rounded-xl border border-slate-200 dark:border-slate-800"
                    >
                        ...
                        return (
                          <button
                            ...
                            className={`flex items-center justify-center gap-1 text-[11px] px-2.5 py-1 rounded-lg font-semibold transition-all border focus:ring-2 focus:ring-olive-500 ${
                              isSelected
                                ? cfg.activeClass
                                : "border-transparent text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-white dark:hover:bg-slate-800"
                            }`}
                          >
                            {cfg.icon}
                            <span>{cfg.label}</span>
                    ...
                    <button
                      ...
                      className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-xl border font-semibold transition-all focus:ring-2 focus:ring-olive-400 ${
                        u.role === "ADMIN" || u.role === "PARTNER" || u.canViewPrivate
                          ? "bg-olive-100 dark:bg-olive-950/80 border-olive-500 text-olive-900 dark:text-olive-200 font-bold"
                          : "bg-slate-100 dark:bg-slate-950 border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-500 hover:text-slate-900 dark:hover:text-slate-300"
                      }`}
                    >
        ...
        <div className="mt-6 pt-4 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between">
          <span className="text-xs text-slate-600 dark:text-slate-400">
            Adresă de login secretă: <code className="text-olive-700 dark:text-olive-300">/portal</code>
          </span>
          <button
            onClick={onClose}
            aria-label="Închide panoul de gestiune utilizatori"
            className="px-4 py-2 rounded-xl bg-slate-200 hover:bg-slate-300 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-900 dark:text-white text-xs font-semibold focus:ring-2 focus:ring-olive-500"
          >
            Închide
          </button>
        </div>
